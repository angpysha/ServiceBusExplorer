...
**Status**: Approved requirements — amended 2026-07-17 for optional native-vault SAS persistence, compact duration editing, the first internal version milestone, and first-internal human-review corrections
...
6. **Given** any duration or true numeric input reachable on a current modern page or dialog, **When** the evaluator views or edits it, **Then** durations use the approved compact duration field and structured popover, while counts and sizes use a separate consistent numeric pattern with visible labels, complete digits and controls, direct typing, sensible limits and increments, accessible guidance, and no clipping at 100%, 150%, or 200% scaling.
...
11. **Given** the evaluator opens the current Send composer, **When** they review each editable field, **Then** concise visible and programmatic guidance states its meaning, expected format or unit, and Azure effect; body and message count are marked required, session ID and schedule delay state their conditional requirements, optional SDK properties remain optional, and the actual queue or topic destination remains explicit.
...
- **FR-044**: In the first internal version, the approved hybrid DurationEditor defined by FR-033 and FR-036–FR-041 MUST replace the broken stepper on every duration input reachable in the inventory of current modern pages and dialogs, including relative scheduling inputs whose whole-millisecond value fits the shared duration model; no inventoried duration field may retain a raw numeric or legacy duration control.
...
- **FR-047**: Every remaining true numeric input on a current modern page or dialog MUST use one shared numeric presentation pattern without being reinterpreted as a duration. Each field MUST retain a visible semantic label, direct keyboard entry, an appropriate whole-number minimum, maximum, and increment, a programmatic name and help description, and enough width for its longest supported value plus increment controls at 100%, 150%, and 200% scaling.
- **FR-048**: The current Send composer MUST provide concise visible guidance and matching programmatic name, help, and required-state semantics for every editable field it exposes. Guidance MUST state meaning, expected format or unit, and Azure effect; visibly distinguish required, conditionally required, and optional values; keep optional Azure SDK properties optional; identify the actual queue or parent-topic destination; and truthfully list richer message properties not yet exposed by the internal composer.
...
- **SC-019**: A reviewed inventory accounts for 100% of duration and true numeric inputs on current modern pages and dialogs. Every duration uses the approved hybrid DurationEditor, every count or size uses the separate shared numeric pattern, and all inventoried inputs pass minimum-width, 100%/150%/200% scaling, keyboard, accessibility, range, and no-obscured-value regression checks.
...
- **SC-022**: Automated inspection accounts for 100% of fields in the current Send composer and finds a visible status label, concise guidance, programmatic name/help, and correct required-state semantics for each; empty required body content blocks submission without losing the draft, and no optional field becomes mandatory.
...
- “Current modern pages and dialogs” means every Avalonia surface registered in application composition or reachable through current navigation, including still-present non-Service-Bus preview folders. The regression evidence records surfaces with editable duration or numeric fields and records reachable surfaces with none, so omissions are measurable.
